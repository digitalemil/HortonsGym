...
public class XMLDocSolrIndexer extends BaseRichBolt {
	...
	public XMLDocSolrIndexer(String solr) {
		SOLRURL = solr;
	}
	...
	private boolean post(String serverUrl, String json) {
		...
		try {
			...
			System.err.println("Error Indexing XML: "+json);
			e.printStackTrace();
			return false;
		}
		connection.disconnect();
...
//		String xml = new String((byte[]) tuple.getValueByField(fields.get(0)));
/*
		if(xml.startsWith("{")) {
			xml= xml.replace("{\"xml\":\"", "");
			xml= xml.replace("}","");		
...
*/
		...
		System.out.println("SolrIndexer: "+xml);
		if(post(url, xml)) {
			collector.ack(tuple);
		}
		else {
